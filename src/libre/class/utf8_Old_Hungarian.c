...
struct fsm *
utf8_Old_Hungarian_fsm(const struct fsm_options *opt)
{
	struct fsm *fsm;
	size_t i;
	...
	if (!fsm_addedge_literal(fsm, s[2], s[3], 0xb2)) { goto error; }
	if (!fsm_addedge_literal(fsm, s[2], s[4], 0xb3)) { goto error; }
	for (i = 0x80; i <= 0xb2; i++) {
