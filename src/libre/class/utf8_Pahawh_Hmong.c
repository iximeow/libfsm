...
struct fsm *
utf8_Pahawh_Hmong_fsm(const struct fsm_options *opt)
{
	struct fsm *fsm;
	size_t i;
	...
	if (!fsm_addedge_literal(fsm, s[2], s[3], 0xac)) { goto error; }
	if (!fsm_addedge_literal(fsm, s[2], s[4], 0xad)) { goto error; }
	if (!fsm_addedge_literal(fsm, s[2], s[5], 0xae)) { goto error; }
	for (i = 0x80; i <= 0xbf; i++) {
